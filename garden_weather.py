...
class GardenWeatherBot(fp.PoeBot):
    ...
    async def get_response(
        ...
        if current_state == UserState.GREETING:
            ...
            async for response in self._handle_watering_preference(user_id, user_message):
                yield response
        elif current_state == UserState.SUNLIGHT_PREFERENCE:
            async for response in self._handle_sunlight_preference(user_id, user_message):
                yield response
        elif current_state == UserState.RECOMMENDATION:
            async for response in self._handle_recommendation_response(user_id, user_message):
                yield response
        else:
            async for response in self._handle_general_chat(user_id, user_message):
                yield response

    async def _handle_greeting(self, user_id: str, message: str) -> AsyncIterable[fp.PartialResponse]:
...
*Remember: I can also give you weather updates for your city - just ask about the weather anytime!* ☀️🌧️"""
    ...
    async def _handle_experience_level(self, user_id: str, message: str) -> AsyncIterable[fp.PartialResponse]:
        """Handle experience level selection"""
        message_lower = message.lower()
        
        if "newbie" in message_lower or "beginner" in message_lower or "new" in message_lower:
        ...
        elif "pro" in message_lower or "experienced" in message_lower or "advanced" in message_lower:
            ...
            self.user_states[user_id] = UserState.WATERING_PREFERENCE
        elif "not sure" in message_lower or "don't know" in message_lower or "help" in message_lower:
...
Which sounds more like you?"""
    ...
    async def _handle_watering_preference(self, user_id: str, message: str) -> AsyncIterable[fp.PartialResponse]:
        """Handle watering preference"""
        message_lower = message.lower()
        
        if "constant" in message_lower or "regular" in message_lower or "often" in message_lower:
        ...
        elif "ignore" in message_lower or "forget" in message_lower or "low" in message_lower or "drought" in message_lower:
    ...
    async def _handle_sunlight_preference(self, user_id: str, message: str) -> AsyncIterable[fp.PartialResponse]:
        """Handle sunlight preference and provide recommendation"""
        message_lower = message.lower()
        
        if "lot" in message_lower or "high" in message_lower or "bright" in message_lower or "yes" in message_lower:
            self.user_preferences[user_id]["sunlight"] = "high"
        elif "low" in message_lower or "little" in message_lower or "shade" in message_lower or "no" in message_lower:
...
**What do you think – are you interested in this plant?** 🤔"""
    ...
    async def _handle_recommendation_response(self, user_id: str, message: str) -> AsyncIterable[fp.PartialResponse]:
        """Handle user's response to plant recommendation"""
        message_lower = message.lower()
        
        if any(word in message_lower for word in ["yes", "interested", "love", "great", "perfect", "want"]):
...
Happy planting! 🌿 And remember, I'm always here if you need weather updates for your green friends! ☀️🌧️"""
            ...
            for key, plant in plant_db.items():
                if f"{preferences['watering']}_water_{preferences['sunlight']}_sun" == key:
...
This one might be more your style! **What do you think about this option?** 🌿"""
    ...
    async def _handle_general_chat(self, user_id: str, message: str) -> AsyncIterable[fp.PartialResponse]:
        """Handle general plant or weather questions"""
        message_lower = message.lower()
        
        plant_keywords = ["plant", "water", "care", "grow", "leaf", "soil", "pot", "garden"]
...
What can I help you with today? 🌱☀️"""
    ...
    async def _handle_weather_request(self, message: str) -> AsyncIterable[fp.PartialResponse]:
